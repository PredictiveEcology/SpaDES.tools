...
#    `functionName` is a function's name in your module (e.g., `prepInputsProblemsEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/Tati/Documents/GitHub/repex/cropRepLCC05-prepInputs/prepInputsProblems/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("prepInputsProblems")
  path <- list(modulePath = "C:/Users/Tati/Documents/GitHub/repex/cropRepLCC05-prepInputs",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    prepInputsProblems = list(.saveInitialTime = NA)
  )
  ...
  if (exists("prepInputsProblemsEvent1", envir = .GlobalEnv)) {
    simOutput <- prepInputsProblemsEvent1(mySim)
  } else {
  ...
  if (exists("prepInputsProblemsEvent2", envir = .GlobalEnv)) {
    simOutput <- prepInputsProblemsEvent2(mySim)
  } else {
